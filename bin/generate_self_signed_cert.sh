...
if [[ ! -f "${SECRETS_DIR}/tls.crt" || ! -f "${SECRETS_DIR}/tls.key" ]]; then
  ...
  openssl req -new -x509 -days 365 -key ${SECRETS_DIR}/ca.key -subj "/C=US/ST=California/L=Pasadena/O=KubeSail-User/OU=Sync/CN=kubesail-sync.local" -out ${SECRETS_DIR}/ca.crt
  ...
  openssl x509 -outform der -in ${SECRETS_DIR}/tls.crt -out ${SECRETS_DIR}/tls.der
